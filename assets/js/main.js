...
!(function($) {
  ...
  $(document).on('click', '.nav-menu a , .imageDiv a , .scrollto', function(e) {
...
setTimeout(()=>{document.querySelector(".alert.alert-success").classList.remove("show")
...
if (!errorSpan && e.target.value.length <5){
  e.target.insertAdjacentHTML('afterEnd' , "<span class='text-danger'>Required at least 5 charachter </span>")
}
if(errorSpan && e.target.value.length >=5)
errorSpan.remove()
}
...
if (!errorSpan && !(/^.+@gmail\.com$/i.test(e.target.value))){
  e.target.insertAdjacentHTML('afterEnd' , "<span class='text-danger'>It must be in form examble@gmail.com </span>")
}
if(errorSpan && (/^.+@.+\.com$/i.test(e.target.value)))
errorSpan.remove()
}
...
document.getElementById("email").addEventListener('blur' , checkEmail)
  ...
  let erroSpang = Array.from(document.querySelectorAll(".email-form .form-group span"))
  let notValid =erroSpang.some((e)=>e.textContent)
  submitButton.setAttribute('disabled' , 'disabled')
  let formFilled = Array.from(document.querySelectorAll(".email-form input")).every((input)=> input.value)
  Array.from(document.querySelectorAll(".email-form input")).forEach((ele)=>{
  ele.addEventListener("change" , ()=>{
    erroSpang = Array.from(document.querySelectorAll(".email-form .form-group span"))
    notValid =erroSpang.some((e)=>e.textContent)
    formFilled = Array.from(document.querySelectorAll(".email-form input")).every((input)=> input.value)
      if(formFilled===false || notValid===true || document.getElementById("message").value===""){
      submitButton.setAttribute('disabled' , 'disabled')
    }
    else{
